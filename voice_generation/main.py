...
if config.ENGINE == "kokoro":
    from kokoro import create

elif config.ENGINE == "qwen":
    from qwen import create
...
DEFAULT_VOICES = {
    "hi": "hm_omega",
    "en-us": "am_michael",
    "fallback": VOICES[DEFAULT_VOICE]["voice"]
}
...
def detect_language_and_voice(text, selected_voice=None):
    ...
    if re.search(r'[\u0900-\u097F]', text):
        ...
        for voice in VOICES.values():
            ...
            if voice["lang"] == "hi":
                return voice["voice"], "hi"

    # First English voice
    for voice in VOICES.values():

        if voice["lang"] == "en-us":
            return voice["voice"], "en-us"

    # Fallback
    first = next(iter(VOICES.values()))

    return first["voice"], first["lang"]
...
def generate_chunk(task):
    ...
    voice, lang_code = detect_language_and_voice(
    text,
    speech_plan.get("voice")
)
    ...
    # User-selected Qwen style instruction
    instruct = speech_plan.get("instruct", "")
...
def main(
    ...
    if custom_text:
        ...
        output_file = os.path.join(OUTPUT_DIR, custom_output)
    ...
    for i, sentence in enumerate(sentences):
        ...
        speech_plan["voice"] = selected_voice or DEFAULT_VOICE

# User-selected Qwen style instruction
        speech_plan["instruct"] = instruct
